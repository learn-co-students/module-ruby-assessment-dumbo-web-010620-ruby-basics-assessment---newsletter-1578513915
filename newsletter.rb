...
def print_one_article(article)
  ...
  #I can use .each to grab each hash element inside the array and
  #puts out the value of each desired key
    article = ARTICLES
    one_article = article.map do |article_hashes| 
      puts "#{article_hashes[:title]},  by: #{article_hashes[:author]},#{article_hashes[:text]}"
    end
end

def print_many_articles(articles)
  articles = ARTICLES
  print_one_article(articles).each do |article|
    puts article
  end
